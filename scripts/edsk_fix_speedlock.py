...
def main(argv):
    ...
    in_dat = list(in_dat[:256])
    t0_sz = struct.unpack("B", in_dat[52])[0]
    in_dat[52] = struct.pack("B", t0_sz + 4) # extra 4*256 bytes
    # Modify the track-0 info to reflect modified Gap3 and weak sector 2
    trk0_dat = list(trk_dat[:256])
    trk0_dat[22] = struct.pack("B", 82)  # Gap3 = 82
    trk0_dat[24+8+7] = struct.pack("B", 6) # Sector 2 actual length == 3*512
    # We are satisfied. Add the necessary info to the output file.
    out_f = open(argv[2], "wb")
    out_f.write("".join(in_dat))   # Image Header
    out_f.write("".join(trk0_dat)) # Trk 0 header
